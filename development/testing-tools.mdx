---
title: "XMage Test Mode and Development Testing Tools"
...
<Warning>
  Test mode disables deck validation, authentication, and connection security checks. **Never run a server with `-DtestMode=true` (or `-testMode=true`) on a network-accessible or public-facing host.** Use it only on a local development machine.
</Warning>
...
<Tip>
  For full GUI and rendering debugging, launch the client with the `-debug` argument **in addition to** running the server in test mode. The debug main menu for GUI and rendering tests is only shown in the client when both conditions are met — test mode on the server **and** `-debug` on the client.
</Tip>
