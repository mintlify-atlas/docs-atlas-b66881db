---
title: "XMage Developer Utility Scripts Reference"
sidebarTitle: "Utility Scripts"
description: "Reference for XMage's Perl and Python utility scripts in Utils/: card generation, set tracking, data import, and release tooling."
---
...
<Note>
  Keep `mtg-cards-data.txt` up to date by running `mtg-cards-data-scryfall.py` after each new Magic set is officially released on Scryfall. The script considers the file stale after 12 hours and will re-download automatically, but you can force a refresh at any time by deleting the cached copy before running.
</Note>
...
Downloads card data from the Scryfall bulk-data API and writes it to `mtg-cards-data.txt` in the pipe-delimited format that all `gen-*.pl` scripts expect. The script handles Scryfall's paginated API automatically and respects the `User-Agent` header requirement (`mage-scryfall-sync/1.0`). It also checks whether the existing file is stale (older than 12 hours) before re-downloading.
